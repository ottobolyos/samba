...
def toggle_daic_mode() -> str:
    ...
    # If switching to implementation mode, check for active task
    if current_mode == "discussion":
        task_state = get_task_state()
        if not task_state.get("task"):
            # No task selected - provide helpful error
            msg = "[DAIC Error] Cannot enter implementation mode without an active task.\n\n"
            ...
            if tasks_dir.exists():
                ...
                if task_files:
                    msg += "Available tasks:\n"
                    for task_file in task_files[:5]:  # Show first 5 tasks
                        msg += f"  • {task_file.stem}\n"
                    if len(task_files) > 5:
                        msg += f"  ... and {len(task_files) - 5} more\n"
                    msg += "\n"

            msg += "To select a task:\n"
            msg += "1. Update .claude/state/current_task.json with the task name\n"
            msg += "2. Or create a new task following sessions/protocols/task-creation.md"
            return msg
