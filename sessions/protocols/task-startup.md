...
1. First check current status to ensure no uncommitted changes
   - Run `git status -s` and address EVERY file shown
...
>   - Address EVERY file shown in `git status -s`, not just expected files
...
**IMPORTANT: DAIC Mode Check**
If you are in discussion mode and the user asks you to select a task:
- STOP before attempting to edit .claude/state/current_task.json
- List available tasks from sessions/tasks/ directory (excluding TEMPLATE.md)
- Inform the user that task selection requires implementation mode
- Ask them to run `daic` to switch to implementation mode first
- Do NOT proceed with implementation suggestions while in discussion mode
